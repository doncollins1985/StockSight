...
def get_device():
    """
    Returns the appropriate device for training (CUDA, MPS, or CPU).
    """
...
def save_scalers(feature_scaler: object, target_scaler: object, scaler_file_X: str, scaler_file_y: str) -> None:
    """
    Save scalers to files using joblib.
    """
    try:
        os.makedirs(os.path.dirname(scaler_file_X), exist_ok=True)
        os.makedirs(os.path.dirname(scaler_file_y), exist_ok=True)
        joblib.dump(feature_scaler, scaler_file_X)
        joblib.dump(target_scaler, scaler_file_y)
...
def load_scalers(scaler_file_X: str, scaler_file_y: str) -> tuple:
    """
    Load feature and target scalers from files using joblib.
    """
    try:
        feature_scaler = joblib.load(scaler_file_X)
        target_scaler = joblib.load(scaler_file_y)
